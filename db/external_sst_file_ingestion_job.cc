  ...
  *assigned_seqno = 0;
  ...
  for (int lvl = 0; lvl < cfd_->NumberLevels(); lvl++) {
    ...
    if (IngestedFileFitInLevel(file_to_ingest, lvl)) {
      ...
      ROCKS_LOG_INFO(db_options_.info_log, "can't ingest into bottom level due to range overlap %d", cfd_->NumberLevels() - 1);
    }
  }

  auto origin_target_level = target_level;
  while (target_level >= vstorage->base_level()) {
    if (vstorage->MaxStaticBytesForLevel(target_level) <= file_to_ingest->file_size + vstorage->NumLevelBytes(target_level) || 
  ...
  if (target_level == 0) {
    ...
    ROCKS_LOG_INFO(db_options_.info_log, "ingest to origin level %d", vstorage->base_level());
