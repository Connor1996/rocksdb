...
Status VersionSet::DumpManifest(Options& options, std::string& dscname,
                                bool verbose, bool hex, bool json, uint64_t sst_file_number) {
  ...
  if (s.ok()) {
    ...
    for (auto cfd : *column_family_set_) {
      ...
      if (sst_file_number != 0) {
        for (int level = 0; level < v->storage_info_.num_levels_; level++) {
          const std::vector<FileMetaData*>& files = v->storage_info_.files_[level];
          for (size_t i = 0; i < files.size(); i++) {
            if (files[i]->fd.GetNumber() == sst_file_number) {
              ...
              printf("%s at level %d\n", files[i]->DebugString(hex).c_str(), level);
              break;
            }
          }
          if (found) break;
    ...
    if (sst_file_number != 0 && !found) {
      s = Status::Corruption("sst " + ToString(sst_file_number) + " is not in the live files set of the manifest");
    }
