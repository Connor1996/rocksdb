    ...
    for (size_t i = 0; i < storage_info_.LevelFilesBrief(0).num_files; i++) {
      ...
      if (!status.ok() || *overlap) {
        if (*overlap) {
          ...
          ROCKS_LOG_INFO(info_log_, "overlap in L%d, range [%s, %s], seek: %s", level, smallest_user_key.ToString(true).c_str(), largest_user_key.ToString(true).c_str(), seek_result.DebugString(true).c_str());
        }
        break;
      }
    }
